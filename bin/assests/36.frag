...
void main()
{
    ...
    float addglow = 0.6;
    {
        ...
        if(dot(n1, n2)<maxs || dot(n1, n3)<maxs) {
            glow += addglow;
        }
    }
    {
    ...
    // Couleur du fond
    const float slt = 0.15;
    // Couleur avancé
    const float multi = 1.75;
    gl_FragColor = vec4(vec3(slt+glow*multi, slt+glow*multi, 0.2+glow)*fog + fog2, 1.0);
}