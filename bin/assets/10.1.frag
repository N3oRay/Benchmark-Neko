 ...
 * Enhanced: Added distance-based fog, higher precision, and lighting (diffuse & specular).
 */
...
// Estimate normal at position p using central differences
...
// Lighting model (diffuse and specular)
vec3 applyLighting(vec3 p, vec3 normal, vec3 viewDir, vec3 baseColor) {
    vec3 lightPos = vec3(3.0, 2.0, -3.0) + vec3(sin(iTime)*2.,cos(iTime),cos(iTime*.5)*2.);
    ...
    // Add some ambient
    float ambient = 0.20;
    return baseColor * (diff + ambient) + lightColor * 0.4 * spec;
}

// Raymarching: find hit, return distance, hit flag, hit position, and normal
void intersect(
    vec3 ro, vec3 rd,
    out float marchDist,
    out bool hit,
